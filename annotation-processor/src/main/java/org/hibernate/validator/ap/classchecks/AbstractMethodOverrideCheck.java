	...
	public Collection<ConstraintCheckError> checkMethod(ExecutableElement currentMethod) {
		...
		for ( ExecutableElement overriddenMethod : overriddenMethods ) {
			if ( !checkOverriddenMethod( currentMethod, overriddenMethod ) ) {
				return CollectionHelper.asSet( new ConstraintCheckError( currentMethod, null, getErrorMessageKey(), getEnclosingTypeElement( overriddenMethod ).getQualifiedName().toString() ) );
